...
import massage.check.config.auth.UserAdapter;
import massage.check.domain.post.PostCategory;
import massage.check.service.comment.CommentService;
import massage.check.service.post.PostCategoryService;
import massage.check.service.post.PostService;
...
public class PostController {
    ...
    public String readAll(@PathVariable(required = false) String postCategory_name,
                          ...
                          @RequestParam(required = false,defaultValue = "0",value = "orderby") String order_criteria,
                          Pageable pageable,
        ...
        /**
         * 클라이언트 페이징에서 받은 page_no와 실제 접근페이지는 다르다.Page객체는 페이지가 0부터 시작
         * 따라서 실제 접근 페이지는 page_no - 1 처리해주어야 한다.
         */
        page_no = (page_no == 0 ) ? 0 : (page_no - 1);

        Page<PostDto.ResponsePageDto> postPageList = postService.getPageList(pageable,page_no,postCategory_name,order_criteria); //페이지 객체 생성
    ...
    public String readById(@PathVariable("post_id") Long post_id,
     ...
     /* 현재 로그인 한 유저가 이 게시물을 좋아요 했는지 안했는지 여부 확인*/
    ...
    public String searchByCategory(@PathVariable("postCategory_name") String postCategory_name,
        ...
        /**
         * 클라이언트 페이지에서 받은 page_no 와 실제 접근 페이지는 다르다. Page객체는 페이지가 0부터 시작
         * 따라서 실제 접근 페이지는 page_no -1 처리 해주어야 한다.
         */
        page_no = (page_no == 0 )? 0 : (page_no -1);

        //페이지 객체 생성
        Page<PostDto.ResponsePageDto> postResponsePageDto = postService.searchPageList(pageable, page_no, keyword, postCategory_name, order_criteria);
    ...
    public String readMyPost(@PathVariable String postCategory_name,
        ...
        /**
         * 클라이언트 페이지에서 받은 page_no와 실제 접근 페이지는 다르다. Page 객체는 페이지가 0부터 시작
         * 따라서 실제 접근 페이지는 page_no - 1 처리해주어야 한다.
         */
        page_no = ( page_no ==0) ? 0 : (page_no - 1);

        Page<PostDto.ResponsePageDto> postResponsePageDto = postService.getMyPostPageList(pageable, page_no, user.getMemberDto().getId(), postCategory_name);
